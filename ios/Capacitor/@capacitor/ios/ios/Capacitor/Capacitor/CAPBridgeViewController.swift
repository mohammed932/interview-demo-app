...
open class CAPBridgeViewController: UIViewController, CAPBridgeDelegate, WKScriptMessageHandler, WKUIDelegate, WKNavigationDelegate {
    ...
    private func getStartPath() -> String? {
        ...
        if assetsFolder == "public" {
            if let assets = defaults.string(forKey: "assetsFolder") {
                assetsFolder = assets
                resourcesPath = assetsFolder
            }
        }
        
        
        print(">> Assets folder: \(assetsFolder)")
//        if !startDir.isEmpty {
...
//        }
        ...
        if !isDeployDisabled() && !isNewBinary() {
            ...
            if (persistedPath != nil && !persistedPath!.isEmpty) {
                ...
                resourcesPath = (snapshots as NSString).appendingPathComponent((persistedPath! as NSString).lastPathComponent)
            }
        }
        
        self.basePath = resourcesPath
