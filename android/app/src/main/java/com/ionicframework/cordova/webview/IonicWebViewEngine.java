...
public class IonicWebViewEngine extends SystemWebViewEngine {
    ...
    @Override
    public void init(CordovaWebView parentWebView, CordovaInterface cordova, final Client client,
        ...
        if (parser.getHostPath() != null && !parser.getHostPath().equals("")) {
            if (parser.getHostPath().contains("najiz")) {
                localServer.hostFiles(parser.getHostPath());
            } else {
                localServer.hostAssets(parser.getHostPath());
            }
        } else {
